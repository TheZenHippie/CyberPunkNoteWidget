<#
.SYNOPSIS
    Builds, publishes, and packages releases for CyberPunkNoteWidget for GitHub.

.DESCRIPTION
    Compiles both:
      1. Standalone / Self-Contained single-file Windows x64 binary (runs standalone without requiring .NET 8 runtime)
      2. Framework-Dependent single-file Windows x64 binary (ultra-compact, requires .NET 8 runtime)
    Packages both into release zip archives with documentation, icon assets, and computes SHA256 checksums.
...
Write-Host "  CyberPunkNoteWidget Release Builder - $tagName" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
dotnet publish "$rootDir\CyberPunkNoteWidget.csproj" `
    -c Release `
    -r win-x64 `
    --self-contained true `
...
# Clean up debug symbols (.pdb) from release output
Remove-Item "$standalonePublishDir\*.pdb", "$frameworkDepPublishDir\*.pdb" -Force -ErrorAction SilentlyContinue

# Copy documentation and assets into publish folders
Write-Host "`n[4/5] Copying branding and documentation assets..." -ForegroundColor Green
...
$frameworkDepZip = Join-Path $releaseDir "CyberPunkNoteWidget-$tagName-win-x64-FrameworkDependent.zip"
$standaloneZip = Join-Path $releaseDir "CyberPunkNoteWidget-$tagName-win-x64-Standalone.zip"
...
Get-ChildItem -Path "$releaseDir\*.zip" | ForEach-Object {
